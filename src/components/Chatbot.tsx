...
export function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Hello! I'm the DevdigitaX AI. How can I help you today?" },
  ]);
  ...
  const generateContext = () => {
    ...
    return `Official DevdigitaX AI. Services: ${services}. Tone: Professional and helpful.`;
  };
  ...
  return (
    <div className="fixed bottom-8 right-32 z-[100] font-sans">
      {!isOpen ? (
        <button
          onClick={() => setIsOpen(true)}
          className="flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-xl transition-all hover:scale-110 active:scale-95"
        >
          <MessageSquare className="h-7 w-7" />
          <div className="absolute top-0 right-0 h-4 w-4 bg-emerald-500 rounded-full border-2 border-[#0A0A0C]" />
        </button>
      ) : (
        <div 
          className={`bg-[#0A0A0C] border border-white/10 shadow-2xl rounded-3xl overflow-hidden flex flex-col transition-all duration-300 ${
            isMinimized ? "h-16 w-72" : "h-[550px] w-[400px] max-h-[calc(100vh-100px)] max-w-[calc(100vw-40px)]"
          }`}
              ...
              <div className="p-5 border-t border-white/5 bg-white/[0.02]">
                <div className="relative flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-blue-500/50 transition-all placeholder:text-zinc-600"
                  />
                  <button
                    onClick={() => handleSend()}
                    disabled={!input.trim() || isLoading}
                    className="h-11 w-11 bg-blue-600 text-white rounded-xl flex items-center justify-center hover:bg-blue-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
