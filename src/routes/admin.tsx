...
import { FileText, Briefcase, Users, LayoutDashboard, Globe, Download, Upload, Search, Edit3, Trash2, Menu, X, ChevronRight } from "lucide-react";
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center gap-3 mb-10 justify-center">
          ...
          <div>
            ...
            <div className="text-[9px] font-black text-blue-500 uppercase tracking-widest">Admin Panel</div>
          </div>
        </div>
...
function AdminPage() {
    const [authed, setAuthed] = useState(() => sessionStorage.getItem("devdigitax_admin_auth") === "true");
    ...
    const loadData = async () => {
      try { setArticles(getArticles() ?? []); } catch { setArticles([]); }
      try { setServices(getServices() ?? []); } catch { setServices([]); }
      try { setPortfolio(getPortfolio() ?? []); } catch { setPortfolio([]); }
      try { setTeam(getTeam() ?? []); } catch { setTeam([]); }
      try { setConfig(getSiteConfig() ?? {}); } catch { setConfig({}); }
      try {
        const serverLogs = await getVisitorsServer();
        setVisitors(Array.isArray(serverLogs) && serverLogs.length > 0 ? serverLogs : (getVisitors() ?? []));
      } catch {
        try { setVisitors(getVisitors() ?? []); } catch { setVisitors([]); }
      }
    };
    ...
    const save = () => {
      ...
      if (!label) { showNotify(isTeamTab ? "Name required" : "Title required"); return; }
    ...
    const del = (id: any) => {
      ...
      try {
        ...
        if (tab === "articles") { const n = filter(articles); setArticles(n); localStorage.setItem("devdigitax_articles", JSON.stringify(n)); }
        else if (tab === "services") { const n = filter(services); setServices(n); saveServices(n); }
        else if (tab === "portfolio") { const n = filter(portfolio); setPortfolio(n); savePortfolio(n); }
        else if (tab === "team") { const n = filter(team); setTeam(n); saveTeam(n); }
        showNotify("Deleted");
      } catch { showNotify("Delete failed"); }
    };
    ...
    const exportData = () => {
      try {
        const data = { articles, services, portfolio, team, config, timestamp: new Date().toISOString() };
      ...
      } catch { showNotify("Export failed"); }
    };
    ...
    const importData = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (Array.isArray(data.articles)) { setArticles(data.articles); localStorage.setItem("devdigitax_articles", JSON.stringify(data.articles)); }
          if (Array.isArray(data.services)) { setServices(data.services); saveServices(data.services); }
          if (Array.isArray(data.portfolio)) { setPortfolio(data.portfolio); savePortfolio(data.portfolio); }
          if (Array.isArray(data.team)) { setTeam(data.team); saveTeam(data.team); }
          if (data.config) { setConfig(data.config); saveSiteConfig(data.config); }
          showNotify("Restored successfully");
        } catch { showNotify("Import failed - invalid file"); }
      };
    ...
    const filteredItems = useMemo(() => {
      ...
      if (tab === "articles") return articles.filter((a: any) => (a.title || "").toLowerCase().includes(q) || (a.content || "").toLowerCase().includes(q));
      if (tab === "services") return services.filter((s: any) => (s.title || "").toLowerCase().includes(q) || (s.description || "").toLowerCase().includes(q));
      if (tab === "portfolio") return portfolio.filter((p: any) => (p.title || "").toLowerCase().includes(q) || (p.client || "").toLowerCase().includes(q));
      if (tab === "team") return team.filter((t: any) => (t.name || "").toLowerCase().includes(q) || (t.role || "").toLowerCase().includes(q));
      if (tab === "visitors") return visitors.filter((v: any) => (v.country || "").toLowerCase().includes(q) || (v.ip || "").toLowerCase().includes(q) || (v.city || "").toLowerCase().includes(q));
      return [];
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
              <h4 className="text-xs font-black uppercase tracking-widest text-zinc-500 mb-3">Recent Activity</h4>
              {visitors.slice(0, 5).map((v: any, i: number) => (
                <div key={v.id || i} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
                  <div className="h-8 w-8 rounded bg-white/10 flex items-center justify-center text-xs font-black">{v.countryCode || "--"}</div>
                  <div className="flex-1 text-xs">{v.country || "Unknown"} – {v.city || "N/A"}</div>
                  <div className="text-[9px] text-zinc-600">{v.timestamp ? new Date(v.timestamp).toLocaleTimeString() : ""}</div>
                </div>
              ))}
              {visitors.length === 0 && <p className="text-xs text-zinc-600 uppercase text-center py-2">No activity</p>}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            {items.map((item: any) => {
              ...
              return (
                <div key={id} className={`p-6 rounded-2xl bg-white/5 border flex items-center gap-4 ${selected.has(id) ? "border-blue-500" : "border-white/10"}`}>
                  <button
                    type="button"
                    onClick={() => toggleSelect(id)}
                    className={`h-5 w-5 rounded border flex-shrink-0 ${selected.has(id) ? "bg-blue-600 border-blue-500" : "border-white/20"}`}
                  />
                  {item.image && <img src={item.image} className="h-12 w-12 rounded-xl object-cover flex-shrink-0" alt="" />}
                  <div className="flex-1 min-w-0">
                    <h4 className="font-black uppercase truncate">{item.title || item.name || "Untitled"}</h4>
                    <p className="text-[10px] text-zinc-500 uppercase tracking-widest truncate">{item.category || item.role || item.description || ""}</p>
                  </div>
      ...
      return (
        <div className="max-w-3xl mx-auto space-y-8 bg-white/5 p-10 rounded-3xl border border-white/10">
          <div className="flex justify-between items-center">
            <button
              type="button"
              onClick={() => { setView("list"); setEditItem(null); setForm({ status: "published" }); }}
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => setForm((f: any) => ({ ...f, status: f.status === "published" ? "draft" : "published" }))}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={e => setForm((f: any) => ({ ...f, [isTeamTab ? "name" : "title"]: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={e => setForm((f: any) => ({ ...f, [isTeamTab ? "role" : "category"]: e.target.value }))}
                  className="w-full bg-black/40 border border-white/10 p-4 rounded-2xl outline-none focus:border-blue-500 transition-colors"
                  placeholder={isTeamTab ? "Role" : "Category"}
                />
              </div>
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Image URL</label>
                <input
                  value={form.image || ""}
            ...
            {tab === "articles" && (
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Content</label>
                <textarea
                  value={form.content || ""}
            ...
            {tab === "portfolio" && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Client</label>
                  <input
                    value={form.client || ""}
                ...
                <div>
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Results</label>
                  <input
                    value={form.result || ""}
                ...
                <div>
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Live Link</label>
                  <input
                    value={form.live || ""}
                ...
                <div>
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Tech Stack</label>
                  <input
                    value={form.tech || ""}
      ...
      <div className="space-y-6">
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setVisitors([]);
                  showNotify("Cleared");
                } catch { showNotify("Failed to clear"); }
              }}
          ...
          {(() => {
            const topCountry = visitors.length
              ? (Array.from(new Set(visitors.map((v: any) => v.country).filter(Boolean))) as string[])
                  .sort((a, b) => visitors.filter((x: any) => x.country === b).length - visitors.filter((x: any) => x.country === a).length)[0] ?? "N/A"
              : "N/A";
            const mobilePct = visitors.length ? `${Math.round((visitors.filter((v: any) => v.isMobile).length / visitors.length) * 100)}%` : "0%";
            const directPct = visitors.length ? `${Math.round((visitors.filter((v: any) => v.referrer === "Direct").length / visitors.length) * 100)}%` : "0%";
            ...
            return [
              ...
              <div key={i} className="p-6 rounded-2xl bg-white/5 border border-white/10">
                <div className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-2">{s.l}</div>
        ...
        <div className="space-y-3">
          {filteredItems.map((v: any, i: number) => (
            <div key={v.id || i} className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-4">
              <div className="h-12 w-12 rounded-xl bg-white/10 flex items-center justify-center font-black text-sm flex-shrink-0">
                {v.countryCode || "--"}
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="font-black uppercase truncate">{v.country || "Unknown"} – {v.city || "N/A"}</h4>
                <p className="text-[10px] text-zinc-500 uppercase truncate">{v.ip || ""} · {v.timestamp ? new Date(v.timestamp).toLocaleString() : ""}</p>
              </div>
              <div className="text-xs text-blue-500 font-mono flex-shrink-0">{v.referrer || "Direct"}</div>
            </div>
          ))}
          ...
          <main className={`${sidebarOpen ? "ml-64" : "ml-0"} transition-all duration-300 p-8`}>
            ...
            <header className="flex items-center justify-between mb-10">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  type="button"
                  onClick={() => { sessionStorage.removeItem("devdigitax_admin_auth"); setAuthed(false); }}
